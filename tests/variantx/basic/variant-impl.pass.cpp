...
#include "tests/variantx/utils/advanced/test-classes.hpp"
#include "tests/variantx/utils/basic/counter.hpp"
...
namespace
{
    ...
    TEST(VariantxImpl, Dtor)
    {
        namespace vi = variantx::impl;
        using variantx::kVariantNpos;

        // Test for TriviallyAvailable
        {
            using Traits = vi::Traits<int, float, advanced_test::Trivial>;
            static_assert(Traits::kDestructibleTrait == vi::Trait::TriviallyAvailable);

            using DtorType = vi::Dtor<Traits>;

            static_assert(std::is_trivially_destructible_v<DtorType>);

            DtorType dtor{std::in_place_index<0>, 42};
            EXPECT_EQ(dtor.Index(), 0);
        }

        // Test for Available with advanced_test::NonTrivialDestructor
        {
            advanced_test::NonTrivialDestructor::reset_counters();

            using Traits = vi::Traits<int, advanced_test::NonTrivialDestructor>;
            static_assert(Traits::kDestructibleTrait == vi::Trait::Available);

            using DtorType = vi::Dtor<Traits>;
            static_assert(std::is_destructible_v<DtorType>);
            static_assert(!std::is_trivially_destructible_v<DtorType>);

            {
                // The constructor for NonTrivialDestructor is not explicit

                DtorType dtor{std::in_place_index<1>};

                EXPECT_EQ(dtor.Index(), 1);
                EXPECT_EQ(advanced_test::NonTrivialDestructor::destructor_count, 0);

            }  // dtor goes out of scope, destructor should be called.

            EXPECT_EQ(advanced_test::NonTrivialDestructor::destructor_count, 1);
        }

        // Test for Available with test_basic::Counter
        {
            using Traits = vi::Traits<int, test_basic::Counter>;
            static_assert(Traits::kDestructibleTrait == vi::Trait::Available);
            ...
            // Test destructor via scope

            std::shared_ptr<test_basic::detail::CounterBlock> cb;
            {
                DtorType dtor{std::in_place_index<1>};  // a Counter is constructed

                auto& counter = vi::access::Base::GetAlternative<1>(dtor);
                cb            = counter.GetCounterBlock();

                test_basic::detail::CounterBlock expected;
                expected.ConstructAdd();

                EXPECT_EQ(*cb, expected);
            }  // Here dtor is destroyed, and the Counter inside is destroyed.

            test_basic::detail::CounterBlock expected_after_destruction;

            expected_after_destruction.ConstructAdd();
            expected_after_destruction.DestructAdd();

            EXPECT_EQ(*cb, expected_after_destruction);
        }
        ...
        // Test for Unavailable
        {
            ...
            using Traits = vi::Traits<int, DeletedDtor>;
            static_assert(Traits::kDestructibleTrait == vi::Trait::Unavailable);
